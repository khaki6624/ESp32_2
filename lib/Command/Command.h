...
struct Command
{
    // مقداردهی Property با operation=SET و Argument اول انجام می‌شود؛ سازگاری دقیق
    // Path و نوع مقدار، مسئول CommandValidator آینده است.
    ...
    // Duration اصلی فقط قرارداد Action موقت است؛ Restore وضعیت قبلی را DurationManager
    // آینده انجام می‌دهد. این مدل Timer، Driver یا وضعیت قبلی را نگهداری/اجرا نمی‌کند.
    // Delay مربوط به Scene Step یا Scheduler باید Argument بماند و اینجا منتقل نشود.
    ...
    bool isValid() const
    {
        if(!context.isValid()||domain==CommandDomain::NONE||!isValidCommandDomain(domain)||
           ...
           argumentCount>COMMAND_MAX_ARGUMENTS||!CommandText::isCanonical(originalText,sizeof(originalText))) return false;
